  ...
  useEffect(() => {
    if (rankingDecisions) {
      const mappedRows = rankingDecisions.map((decision, index) => ({
        ...
        finalizedAt:
          decision.status === "Finalized" ? decision.finalizedAt : "-",
        finalizedBy:
          decision.status === "Finalized"
            ? decision.finalizedBy == null
              ? "N/A"
              : decision.finalizedBy
            : "-",
    ...
    try {
      ...
      setOriginalDecisionName(
        selectedCurrentDecision
          ? selectedCurrentDecision.decisionName
          : editGroup.currentRankingDecision
      );
  ...
  const handleAddRankingDecision = async () => {
    ...
    try {
      ...
      if (error.response && error.response.data) {
        ...
        if (
          detailMessage &&
          detailMessage.includes("RankingGroup name exists already!")
        ) {
          setValidationMessage("RankingGroup name exists already!"); // Chỉ hiển thị thông báo lỗi mong muốn
        } else {
          showErrorMessage(
            "Error occurred adding Ranking Decision. Please try again"
          );
        }
      } else {
  ...
  const handleDeleteRankingDecision = async () => {
    try {
      if (decisionToDelete) {
        ...
        setRankingDecisions(
          rankingDecisions.filter(
            (decision) => decision.decisionId !== decisionToDelete
          )
        );
      ...
      showErrorMessage(
        "Error occurred removing Ranking Decision. Please try again."
      );
      handleCloseDeleteRankingDecisionModal();
    }
  };
        ...
        <>
          ...
          {params.row.status !== "Finalized" && (
            <Button
              ...
              color="error"
              onClick={() =>
                handleOpenDeleteRankingDecisionModal(params.row.id)
              }
            >
              <MdDeleteForever />
            </Button>
  ...
  return (
    <div style={{ marginTop: "60px" }}>
      ...
      <Box sx={{ marginTop: 4, padding: 2 }}>
        ...
        <Typography variant="h5" sx={{ flexShrink: 0, marginRight: "16px" }}>
          Ranking Decision List
        </Typography>
        {/* Search Decision */}
        <SearchComponent
          onSearch={handleSearch}
          placeholder=" Sreach Decision"
        />
        ...
        <Modal
          open={showEditGroupInfoModal}
          onClose={handleCloseEditGroupInfoModal}
        >
          <Box
            sx={{
              padding: 2,
          ...
          >
            ...
            <Autocomplete
              disablePortal
              options={
                listDecisionSearchClone
                  ? listDecisionSearchClone.filter(
            ...
            >
              ...
              <Button variant="contained" onClick={handleEditGroupInfo}>
                Save
              </Button>
            </Box>
          </Box>
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="btn-close"
                aria-label="Close"
                onClick={handleCloseAddRankingDecisionModal}
              ></button>
            </Typography>
            ...
            >
              ...
              <Button variant="contained" onClick={handleAddRankingDecision}>
                Save
              </Button>
            </Box>
          </Box>
